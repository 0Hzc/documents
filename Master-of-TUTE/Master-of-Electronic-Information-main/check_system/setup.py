...
def run_check(config):
    ...
    try:
        ...
        process_hy_data(
            satellite_type,
            hy_file_l2a=os.path.join(input_dir, config[satellite_type]['l2a_file']),
            hy_file_l2b=os.path.join(input_dir, config[satellite_type]['l2b_file']),
        ...
        if source_type == 'XC':
            ...
            create_xc_report(satellite_type,output_dir, output_dir, font_path, time_threshold)
...
def process_xc_timematch(input_dir, output_dir, target_sensor, time_threshold):
    ...
    try:
        ...
        target_files = [f for f in os.listdir(input_dir) 
                       if f.startswith(f"{target_sensor}_") and 
                       any(x in f for x in ['Rrs', 'sst', 'AOT', 'chl', 'CDOM', 'TSM']) and 
                       f.endswith('.txt')]
...
def create_satellite_report(input_path, output_dir, font_path, time_size, space_size):
    ...
    c.save()
def create_xc_report(satellite_type,input_path, output_dir, font_path, time_size):
    """创建星地检验报告PDF"""
    # 基础设置
    satellite_info, source_data, product, time_info = extract_info_from_filenames(input_path)
    satellite_num = satellite_info[2:] if satellite_info.startswith('HY') else 'XX'
    ...
    for report_data in report_data_list:
        ...
        # 处理时间差分布图
        time_image_name = f"timestastic_{satellite_type}_XC_{product}_"
        ...
        # 处理检验结果分布图
        val_image_name = f"valstastic_{satellite_type}_XC_{product}_"
        val_image_path = None
        for file in os.listdir(input_path):
